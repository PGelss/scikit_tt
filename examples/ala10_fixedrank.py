...
# parameters
downsampling_list = np.array([100, 200, 500, 1000, 2000])
# Define lag times by physical times, to make sure the same lag times are used for each data set:
lag_times_phy = 1e-3 * np.array([2000, 4000, 6000, 8000])
lag_times_msm = 1e-3 * np.array([100, 200, 500, 1000, 2000, 4000, 6000])
# Set ranks of SVDs:
rank_svd = 100

# data directory (data not included)
directory = "/Users/fkn1/Documents/Uni/Data/Dihedral_Data/"

# apply tEDMD with HOSVD
timescales_hosvd = np.zeros((len(downsampling_list), len(lag_times_phy), 3))

for i in range(len(downsampling_list)):

    # Select next downsampling rate:
    downsampling_rate = downsampling_list[i]
    ...
    if i==0:
        start_time = utl.progress('Apply AMUSEt (HOSVD, delta_t =' + str("%d" % downsampling_list[i]) + ')', 0)
    eigenvalues, _ = tedmd.amuset_hosvd(data, x_list, y_list, basis_list, threshold=0.0, max_rank=rank_svd)
    for j in range(len(lag_times_phy)):
        timescales_hosvd[i, j, :] = -lag_times_phy[j] / np.log(eigenvalues[j][1:4])
    utl.progress('Apply AMUSEt (HOSVD, delta_t=' + str("%d" % downsampling_list[i]) + ')',
                 100 * (i+1) / len(downsampling_list), cpu_time=_time.time() - start_time)
...
x = lag_times_phy
for i in range(len(downsampling_list)):
    plt.plot(x, timescales_hosvd[i, :, 0], "o-", label="%d" % downsampling_list[i], linewidth=2)
plt.xlim([x[0], x[-1]])
plt.ylim([4, 12])
